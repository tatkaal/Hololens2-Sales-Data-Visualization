...
namespace InfoDev
{
    public class Interactions : MonoBehaviour
    {
        ...
        private GameObject[] InfoDevObjects=null; 
        ...
        public void Init(Vis vis)
        {
            ...
            if (targetVis != null)
            {
                ...
                {
					InfoDevObjects = GameObject.FindGameObjectsWithTag("InfoDev");         //Collect vis objects
                }
        ...
        {
            InteractableToggleCollection checkBoxSet =
               legendGameObject.GetComponent<InteractableToggleCollection>();
            if (checkBoxSet == null) return;
            checkBoxSet.SetSelection(0);
            ...
            for (int i = 0; i < legendGameObject.transform.childCount; i++)
            {
                ...
                if(child.GetComponent<LegendValue>() != null)
                {
                    ...
                    if(box != null)
                    {
                        ...
                        Interactable toggle = 
                            box.gameObject.GetComponent<Interactable>();

                        if(toggle != null)
                        {
                            Microsoft.MixedReality.Toolkit.Physics.Interpolator();
                            checkBoxSet.Interactable.Add(toggle);
        ...
        private void UpdateFilterResultsForCategoryFromLegend(GameObject legendObject, string field, string category)
        {
            ...
            if (targetVis.GetIsLinked())
            {
                for (int i = 0; i < InfoDevObjects.Length; i++)
                {
                    if (InfoDevObjects[i].GetComponent<Vis>().GetIsLinked())
                    {
                        if (InfoDevObjects[i].GetComponent<Vis>().GetDataName() == targetVis.GetDataName())
                        {
							GameObject InfoDevInteraction = InfoDevObjects[i].transform.Find("InfoDevInteractions").gameObject;
                            List<bool> t_res = new List<bool>(new bool[res.Count]);
                            if (!InfoDevInteraction.GetComponent<Interactions>().filterResults.ContainsKey(field))
                            {
                                InfoDevInteraction.GetComponent<Interactions>().filterResults.Add(field, t_res);
                            }
                            for (int b = 0; b < t_res.Count; b++)
                            {
                                if (visibleCategories.Contains(InfoDevObjects[i].GetComponent<Vis>().markInstances[b].GetComponent<Mark>().datum[field]))
                                {
                                    t_res[b] = true;
                                }
                                else
                            ...
                            InfoDevInteraction.GetComponent<Interactions>().filterResults[field] = t_res;
                            InfoDevObjects[i].GetComponent<Vis>().FiltersUpdated();
                        }
                    }
                }
            }
...
//            if (targetVis.GetIsLinked()) Synchronize(field);
        ...
        internal void AddThresholdFilter(JSONObject interactionSpecs)
        {
            ...
            InfoDev.SliderGestureControlBothSide sliderControl =
                thresholdFilterInstance.GetComponent<InfoDev.SliderGestureControlBothSide>();
        ...
        public void ThresholdFilterUpdated()
        {
            ...
            InfoDev.SliderGestureControlBothSide sliderControl =
               selectedObject.GetComponent<InfoDev.SliderGestureControlBothSide>();
                ...
                for (int i = 0; i < InfoDevObjects.Length; i++)
                {
                    if (InfoDevObjects[i].GetComponent<Vis>().GetIsLinked())
                    {
                        if (InfoDevObjects[i].GetComponent<Vis>().GetDataName() == targetVis.GetDataName())
                        {
                            ...
                            for (int b = 0; b < t_res.Count; b++)
                            {
                                if (float.Parse(InfoDevObjects[i].GetComponent<Vis>().markInstances[b].GetComponent<Mark>().datum[field]) >= thresholdMinValue && float.Parse(InfoDevObjects[i].GetComponent<Vis>().markInstances[b].GetComponent<Mark>().datum[field]) <= thresholdMaxValue)
                                {
                                    t_res[b] = true;
                                }
                                else
        ...
        private void Synchronize(string field)
        {
            for(int i = 0; i < InfoDevObjects.Length; i++)
            {
                if (InfoDevObjects[i].GetComponent<Vis>().GetIsLinked())
                {
                    ...
                    {
                        GameObject InfoDevInteraction=InfoDevObjects[i].transform.Find("InfoDevInteractions").gameObject;
                        if (InfoDevInteraction != null)
                        {
                            if (InfoDevInteraction.GetComponent<Interactions>().filterResults.ContainsKey(field))
                            {
                                InfoDevInteraction.GetComponent<Interactions>().filterResults[field] = filterResults[field];
                                InfoDevObjects[i].GetComponent<Vis>().FiltersUpdated();
                            }
                        }
                    }
