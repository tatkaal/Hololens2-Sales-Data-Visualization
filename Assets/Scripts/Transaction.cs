...
namespace HoloToolkit.MRDL.PeriodicTable
{
    ...
    {
        public static Transaction ActiveElement;

        public TextMesh TransactionTotalProfit;
        public TextMesh TransactionDescription;
        public TextMesh TransactionTotalQuantity;

        public SpriteRenderer depImage;

        public TextMeshProUGUI TransactionProductDescription;
        public Text DataTotalTransaction;
        ...
        {
            Transaction element = gameObject.GetComponent<Transaction>();
        ...
        public IEnumerator UpdateActive()
        {
            ...
            //Color elementNameColor = TransactionName.GetComponent<MeshRenderer>().material.color;

            while (Transaction.ActiveElement == this)
            {
                //TransactionName.GetComponent<MeshRenderer>().material.color = elementNameColor;
         ...
         */
        public void SetFromElementData(TransactionData data, Dictionary<string, Material> typeMaterials)
        {
            this.data = data;

            TransactionTotalProfit.text = data.Total_Profit.ToString();
            TransactionDescription.text = data.Description;
            TransactionTotalQuantity.text = data.Total_Quantity.ToString();

            Sprite test = Resources.Load<Sprite>("DepartmentIMG/Cigarettes");
            depImage.sprite = test;
            // Mesh mesh = Resources.Load<Mesh>("HologramMesh/");
            // hologram.mesh = mesh;

            TransactionProductDescription.text = data.Product_Description;
            DataTotalTransaction.text = data.Total_Transaction.ToString();
            ...
            if (!typeMaterials.TryGetValue(data.Department.Trim(), out dimMaterial))
            {
                Debug.Log("Couldn't find " + data.Department.Trim() + " in element " + data.Description);
            }

            // Create a new highlight material and add it to the dictionary so other can use it
            string highlightKey = data.Department.Trim() + " highlight";
            ...
            transform.parent.name = data.Description;
        }
    }
}
